...
import { uploadImage } from '@/components/utils/imageUploadUtils';
...
const API_BASE_URL = (process.env.EXPO_PUBLIC_API_URL || 'http://192.168.1.72:3000') + '/api';
...
class UserService {
  ...
  async uploadProfilePicture(imageUri: string): Promise<{ photo: string }> {
    ...
    try {
      console.log('[UserService] Starting profile picture upload...');

      // Construire l'URL correcte (émulateur vs device physique)
      const uploadUrl = buildApiUrl('/api/users/me/photo', '3000');

      // Note: uploadImage ne supporte pas les headers custom actuellement
      // On doit utiliser fetch directement si on a besoin d'Authorization
      ...
      const response = await fetch(uploadUrl, {
        ...
        headers: {
          'Authorization': token ? `Bearer ${token}` : '',
          // NO Content-Type - let fetch handle it
        },
        body: formData,
      });
      ...
      console.log('[UserService] Upload successful!');
      return result;
    } catch (error) {
      console.error('[UserService] Profile picture upload error:', error);
      throw error;
    }
  }
